...
public class ModalServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (tarefa.equals("modalProdutosServico")) {
                sqlState = "SELECT P.NOME, P.TIPO, P.DESCRICAO, SP.QTD_PRODUTO, SP.VALOR_TOTAL FROM TB_ORDEM_SERVICO_PRODUTOS AS SP\n"
                        + "	INNER JOIN tb_produto AS P ON SP.ID_PRODUTO = P.ID_PRODUTO\n"
                        + " WHERE SP.ID_ORDEM_SERVICO = " + id;
            }

            if (tarefa.equals("modalServico")) {
                sqlState = "SELECT * FROM TB_ORDEM_SERVICO_MANUTENCAO WHERE ID_ORDEM_SERVICO = " + id;
            }
            ...
            if (tarefa.equals("modalServico")) {
                while (rs.next()) {
                    ...
                    relatorio.setidOrdemManutencao((rs.getInt("ID_ORDEM_SERVICO_MANUTENCAO")));
                    relatorio.setdsOrdemServico(rs.getString("DS_SERVICO"));
                    relatorio.setValorOrdem(Double.parseDouble(rs.getString("VALOR_SERVICO")));

                    relatorios.add(relatorio);
                }
            } else if (tarefa.equals("modalProdutosServico")) {
                while (rs.next()) {
                    RelatorioModel relatorio = new RelatorioModel();
                    relatorio.setNomeProduto((rs.getString("P.NOME")));
                    relatorio.setQuantidadeProduto(Integer.parseInt(rs.getString("SP.QTD_PRODUTO")));
                    relatorio.setValorProduto(Double.parseDouble(rs.getString("SP.VALOR_TOTAL")));
                    relatorio.setdescricao(rs.getString("P.DESCRICAO"));
                    relatorio.settipoProduto(rs.getString("P.TIPO"));

                    relatorios.add(relatorio);
                }

            } else if (!tarefa.equals("modelPedidos")) {
                while (rs.next()) {
                    RelatorioModel relatorio = new RelatorioModel();
        ...
        if (tarefa.equals("modalServico")) {
            String modal = "<div id=\"my-modal\" class=\"modal fade\" tabindex=\"-1\">\n"
                    ...
                    + "											<div class=\"modal-header\">\n"
                    + "												<button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">&times;</button>\n"
                    + "												<h3 class=\"smaller lighter blue no-margin\">Serviços</h3>\n"
                    + "											</div>\n"
                    + "\n"
                    + "											<div class=\"modal-body\">\n"
                    ...
                    + "                                    <tbody>\n";
            for (RelatorioModel relatorio : relatorios) {
                modal += "<th>" + relatorio.getidOrdemManutencao() + "</th><th>" + relatorio.getdsOrdemServico() + "</th><th class='row_currency2'>" + relatorio.getValorOrdem() + "</th></tr>";
            }
            modal += "                                    </tbody>\n"
                    + "                                </table>"
                    + "											</div>\n"
